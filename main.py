...
def wlog(string, verbose=False):
    """
    génération d'un fichier log principal suivant l'exécution du script log.txt
    """
...
def new_start(verbose=False):
    """
    Création d'une entête type pour le log
    """
...
def loading_a_dataframe(string_to_the_csv, verbose=False):
    ...
    retourne le dataframe formater
    """
...
def get_basics_info(df_file, verbose=False):
    """ 
    Obtention d'information sommaire sur le jeux de données
    """
...
def get_seasons(verbose=False):
    """ 
    Renvois de la saisons en fonction des éphémérides
    """
...
def create_column_seasons(df_file, liste_seasons, verbose=False):
    ...
    df_file["SEASONS"] = liste_column_season
...
def draw_bivariate(name_of_df_file, df_file, custom_palette, switch_sub=False, verbose=False):
    """
    Génère le nuage de point à la façon impressioniste
    """
    ...
    describe_HUM = df_file.HUMIDITY.describe()
    ...
    describe_TMP = df_file.TEMPERATURE.describe()
...
def draw_bivariate_per_seasons(name_of_df_file, df_file, custom_palette, verbose=False):
    """ 
    Séparation des saisons sur un canvas horizontal
    """
...
def draw_dispersion(df_file, name_of_df_file, custom_palette, verbose=False):
    """ 
    active les courbes de dispersion normal
    """
    tps_start = perf_counter()

    # print(df_file.columns)
    df_file.rename(columns={"TEMPERATURE": "Temperatuur", "HUMIDITY": "Relatieve vochtigheid","SEASONS": "Seizoenen"}, inplace=True)
    ...
    sns_plot = sns.jointplot(data=df_file, x="Temperatuur", y="Relatieve vochtigheid", marker=".",hue="Seizoenen", palette=custom_palette)
...
def main(verbose=False):
    ...
    for cpt_file in range(len(liste_fichiers_csv)):
        ...
        switch_sub = False
        custom_palette = ["blue", "green", "red", "orange"] 
